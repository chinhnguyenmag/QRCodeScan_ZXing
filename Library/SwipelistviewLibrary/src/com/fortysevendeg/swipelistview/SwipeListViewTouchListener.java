...
public class SwipeListViewTouchListener implements View.OnTouchListener {
	...
	public boolean onTouch(View view, MotionEvent motionEvent) {
		...
		case MotionEvent.ACTION_MOVE: {
			...
			float maxDeltaXLeft = -viewWidth + leftOffset;
			float maxDeltaXRight = viewWidth - rightOffset;
			...
			if (swiping && downPosition != ListView.INVALID_POSITION) {
				if (opened.get(downPosition)) {
					...
					if (deltaX < 0) {
						if (Math.abs(deltaX) > Math.abs(maxDeltaXLeft) + 20) {
							deltaX = maxDeltaXLeft - 20;
						}
					} else {
						if (Math.abs(deltaX) > Math.abs(maxDeltaXRight) + 20) {
							deltaX = maxDeltaXRight + 20;
						}
					}
				}
				move(deltaX);
